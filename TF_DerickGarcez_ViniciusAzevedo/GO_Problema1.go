...
// Leitores e Escritores (problema 1) - modelo GO
		...
		if s == 0 {
			...
			select {
				case leitorEntra[lx] <- struct{}{}: // entra leitor lx
					s++
					lx++
				case escritorEntra[1] <- struct{}{}: // escritor 1 acessa, escreve e sai
					escritorSai[1] <- struct{}{}
				case escritorEntra[2] <- struct{}{}: // escritor 2 acessa, escreve e sai
					escritorSai[2] <- struct{}{}
			}
		} else if s > 0 && s < QL {
			//se tem pelo menos um leitor acessando (nao todos)
			select {
